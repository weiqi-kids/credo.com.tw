...
// credo 特化（2026-07-21 從舊 scripts/audit-ai-tone.mjs port；核心已含的不重複）：
//   ① 全形括號旁白＝用戶對 credo 的紅線（法律品牌文禁旁白註解）。
//   ② 扮演…角色＝擬人化拔高，核心無。
// 其餘舊 gate 規則（不是/不僅/值得注意/換句話說/綜上所述/隨著…發展/在…的今天/至關重要/
//   模糊引用/模板化開頭/燈塔·見證·里程碑/首先…其次…最後/讓我們·不妨…）核心 ERROR_TELLS
//   與 WARN_LAYERS 已全數涵蓋，不重複加。
const SITE_ERROR_TELLS = [
  ["全形括號旁白（用戶紅線）", /（[^）]*）/],
  ["扮演…角色", /扮演[^。！？\n]{0,12}角色/],
];
const SITE_WARN_LAYERS = { 詞彙: [], 句式: [], 結構: [], 語氣: [] };
// credo 法律固定用語白名單：命中該行不算違規（否則法條/紀年會誤判，且允許括號內的法條引註）。
